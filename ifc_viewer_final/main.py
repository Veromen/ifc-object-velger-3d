...
def main():
    ...
    if st.session_state.filtered_ifc_data is None:
        ...
        if uploaded_file is not None:
            ...
            suffix = "_stories_" + "_".join(stories)
            if filter_option == "IFC Product and Keywords" and ifc_product:
                suffix += f"_product_{ifc_product}"
            if keywords:
                suffix += "_keywords_" + "_".join(keywords)
        ...
        if not patcher or not filtered_ifc_data:
            ...
            if not filtered_products:
                ...
                st.header(f"{st.session_state.uploaded_file_name}")
                ...
                with tempfile.NamedTemporaryFile(suffix=".ifc", delete=False) as filtered_ifc:
                    ...
                    try:
                        ...
                        st.error(f"Error writing filtered IFC to temporary file: {e}")
                        return
                glb_path = os.path.join(tempfile.gettempdir(), glb_filename)
                convert_cmd = f'ifcconvert "{filtered_ifc_path}" "{glb_path}"'
                with st.spinner("Converting IFC to GLB..."):
                    retcode = subprocess.run(convert_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                if retcode.returncode != 0:
                    st.error("Conversion to .glb failed. Make sure ifcconvert is installed and on PATH.")
                    ...
                    html_snippet = textwrap.dedent("""
                        ...
                        <body>
                          ...
                          <script type="module">
                            ...
                            const base64Data = "GLB_BASE64_PLACEHOLDER";
                    ...
                    """)
                    html_snippet = html_snippet.replace("GLB_BASE64_PLACEHOLDER", glb_base64)
